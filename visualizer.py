...
def plot_basics(force_function, results, scenario):
    ...
    subplots[1,0].plot(force_function['loggingTime(txt)'], results['damper_vel_fr'], label='damper speed (m/s, fr)')
    subplots[1,0].plot(force_function['loggingTime(txt)'], results['damper_vel_fl'], label='damper speed (m/s, fl)')
    subplots[1,0].plot(force_function['loggingTime(txt)'], results['damper_vel_rr'], label='damper speed (m/s, rr)')
    subplots[1,0].plot(force_function['loggingTime(txt)'], results['damper_vel_rl'], label='damper speed (m/s, rl)')
    subplots[1,0].set_ylabel('Damper Speed (m/s)')
    subplots[1,0].legend()
    subplots[1,0].grid(True)

    subplots[1,1].plot(force_function['loggingTime(txt)'], results['tire_load_fr'], label='tire load (N, fr)')
    subplots[1,1].plot(force_function['loggingTime(txt)'], results['tire_load_fl'], label='tire load (N, fl)')
    subplots[1,1].plot(force_function['loggingTime(txt)'], results['tire_load_rr'], label='tire load (N, rr)')
    subplots[1,1].plot(force_function['loggingTime(txt)'], results['tire_load_rl'], label='tire load (N, rl)')
    #subplots[1,1].plot(force_function['loggingTime(txt)'], (results['tire_load_fr']+results['tire_load_fl']+results['tire_load_rr']+results['tire_load_rl'])/4, label='tire load (N, avg)')
    subplots[1,1].set_ylabel('Tire Load (N)')
    subplots[1,1].legend()
    subplots[1,1].grid(True)

    subplots[2,0].plot(force_function['loggingTime(txt)'], results['lateral_load_dist_f'], label='lateral load dist (%, f)')
    subplots[2,0].plot(force_function['loggingTime(txt)'], results['lateral_load_dist_r'], label='lateral load dist (%, r)')
    subplots[2,0].set_ylabel('Lateral Load Distribution (%)')
    subplots[2,0].legend()
    subplots[2,0].grid(True)

    subplots[2,1].plot(force_function['loggingTime(txt)'], results['lateral_load_dist_ratio'], label='lateral load dist ratio (%, f)')
