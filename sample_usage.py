...
time, tire_load, damper_vel, roll_angle_f, roll_angle_r = BB.Shaker()

# time_array, c_array, tire_load = BB.Shaker()

print('Graphing...')
#plt.plot(time, Gs, label='lateral acceleration (Gs)')
plt.plot(time, roll_angle_f, label='roll angle front (deg)')
plt.plot(time, roll_angle_r, label='roll angle front (deg)')
#plt.plot(time, tire_load, label='tire load (N)')
plt.legend()
plt.show()
